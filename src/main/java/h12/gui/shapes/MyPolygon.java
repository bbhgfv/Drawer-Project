...
public class MyPolygon extends MyShape {
    ...
    public JSONObject toJSON() {
        JSONObjectNode.JSONObjectEntryNode name = new JSONObjectNode.JSONObjectEntryNode(new JSONStringNode("name"),new JSONStringNode(TYPE.getSpelling()));
        JSONObjectNode.JSONObjectEntryNode edges = new JSONObjectNode.JSONObjectEntryNode(new JSONStringNode("edges"),new JSONNumberNode(this.edges));
        JSONObjectNode.JSONObjectEntryNode x = new JSONObjectNode.JSONObjectEntryNode(new JSONStringNode("x"),new JSONArrayNode(this.y.stream().map(i->new JSONNumberNode(i)).map(i->(JSONElement)i).toList()));
        JSONObjectNode.JSONObjectEntryNode y = new JSONObjectNode.JSONObjectEntryNode(new JSONStringNode("y"),new JSONArrayNode(this.y.stream().map(i->new JSONNumberNode(i)).map(i->(JSONElement)i).toList()));
        JSONObjectNode.JSONObjectEntryNode fillColor = new JSONObjectNode.JSONObjectEntryNode(new JSONStringNode("fillColor"),ColorHelper.toJSON(this.fillColor));
        JSONObjectNode.JSONObjectEntryNode borderColor = new JSONObjectNode.JSONObjectEntryNode(new JSONStringNode("borderColor"),ColorHelper.toJSON(this.borderColor));
        return JSONObject.of(name,edges,x,y,fillColor,borderColor);
    }
