...
public abstract class MyShape {
    ...
    public static MyShape fromJSON(JSONElement element) throws JSONParseException {
        try {
            JSONStringNode shapeTypeString = (JSONStringNode) element.getValueOf("name");
            ShapeType shapeType = ShapeType.fromString(shapeTypeString.getString());
            if (shapeType == null) {
                throw new JSONParseException("Invalid shape type: " + shapeTypeString + "!");
            } else {
                MyShape myShape=null;
                switch (shapeType) {
                    case CIRCLE:
                        myShape=jsonToShapeConverter.circleFromJSON(element);
                    case RECTANGLE:
                        myShape=jsonToShapeConverter.rectangleFromJSON(element);
                    case CUSTOM_LINE:
                        myShape=jsonToShapeConverter.customLineFromJSON(element);
                    case POLYGON:
                        myShape=jsonToShapeConverter.polygonFromJSON(element);
                    case TRIANGLE:
                        throw new JSONParseException("Invalid shape type: " + shapeTypeString + "!");
                    case STRAIGHT_LINE:
                        throw new JSONParseException("Invalid shape type: " + shapeTypeString + "!");
