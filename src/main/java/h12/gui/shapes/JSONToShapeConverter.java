...
public class JSONToShapeConverter {
    ...
    public MyPolygon polygonFromJSON(JSONElement element) throws JSONParseException {
        try {
            List<Integer> x = Arrays.stream(element.getValueOf("x").getArray()).map(i->i.getInteger()).toList();
            List<Integer> y = Arrays.stream(element.getValueOf("x").getArray()).map(i->i.getInteger()).toList();
            ...
            Integer edges = element.getValueOf("edges").getInteger();
